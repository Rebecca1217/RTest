...
<<report_type>>=
## report type ##
rpt_title <- paste0("组合管理跟踪简报(", format(CurrentDate, "%Y.%m.%d"),")")
if_eval <- TRUE
...
<<perfmce_cb>>=
tbAlign <- c("l", "l", rep("r", ncol(perfmce_cb.t) - 1))
perfmce_cb.t %>%
 xtable(., align = tbAlign) %>%
 print.xtable(
   .,
    add.to.row = list(
     pos = list(nrow(perfmce_cb.t) - 5),
     command = " \\midrule \\rowcolor[gray]{0.9} \n"
     )
   )
@
*注：1、当日上证指数点位为：。2、假设：今年管道计划剩余分红率；保持当前回购仓位不变；未来回购平均成本4.50\%；流动性部分年化收益3.00\%；年度平均费用为0BP；假设债基收益为0；债券浮盈浮亏不考虑。
%\fi
\subsection{投连组合}
<<perfmce_ul>>=
tbAlign <- c("l", "l", rep("r", ncol(perfmce_ul.t) - 1))
perfmce_ul.t %>%
 xtable(., align = tbAlign) %>%
 print.xtable(
   # .,
   # add.to.row = list(
   #   pos = list(nrow(perfmce_ul) - 1),
   #   command = " \\midrule \\rowcolor[gray]{0.9} \n"
   #  )
   )
@
